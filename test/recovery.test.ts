...
describe("recovery rules", () => {
  ...
  it("matches Docker not installed (distinct from daemon-down) with a one-line install", () => {
    const r = findRecovery("/bin/sh: docker: command not found");
    expect(r?.ruleId).toBe("docker-not-installed");
    expect(r?.remediation).toMatch(/install docker/i);
    // not-installed must NOT be misread as the daemon being down
    expect(findRecovery("docker: command not found")?.ruleId).not.toBe("docker-daemon-down");
  });

  it("matches a missing compiler toolchain and offers --fix via build-tools", () => {
    const r = findRecovery("make: command not found");
    expect(r?.ruleId).toBe("build-tools-missing");
    expect(r?.systemDeps).toContain("build-tools");
    expect(findRecovery("gcc: error: C compiler cannot create executables")?.ruleId).toBe(
      "build-tools-missing",
    );
  });

  it("falls back to the generic command-not-found rule for an unknown tool", () => {
    const r = findRecovery("bash: foobar-cli: command not found");
    expect(r?.ruleId).toBe("command-not-found");
    expect(r?.remediation).toMatch(/install|package manager/i);
  });

  it("specific rules win over the generic command-not-found (ordering)", () => {
    // pkg-config → openssl rule, not the generic catch-all
    expect(findRecovery("pkg-config: command not found")?.ruleId).toBe("openssl-headers-missing");
    // docker → docker-not-installed, not the catch-all
    expect(findRecovery("docker: command not found")?.ruleId).toBe("docker-not-installed");
    // make → build-tools, not the catch-all
    expect(findRecovery("make: command not found")?.ruleId).toBe("build-tools-missing");
  });

  it("still does NOT match unrelated 'not found' phrasings", () => {
    // No "<tool>: not found" colon form → no generic match.
    expect(findRecovery("npm ERR! 404 Not Found")).toBeNull();
    expect(findRecovery("ENOENT: no such file or directory")).toBeNull();
  });
});
