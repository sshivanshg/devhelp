...
- **Go/Rust/Ruby/Bun/Deno installs now resolve the runtime on a clean machine.** A freshly installed Go (`/usr/local/go/bin`), Rust (`~/.cargo/bin`), rbenv Ruby, Bun (`~/.bun/bin`), or Deno (`~/.deno/bin`) isn't on `PATH` in the fresh shell the deps step spawns, so `go mod download` / `cargo build` / `bundle install` / `bun install` / `deno cache` failed with "command not found". The runtime wrapper now adds the right PATH/init for each (no-op when the tool is already on PATH). Previously only Node and Python were covered. (Bun was the worst case: a bun-only project set no Node version, so its install command was never wrapped at all.)
- **Go now installs correctly on arm64 and modern Go projects.** The Linux installer hardcoded `linux-amd64` (so an arm64 machine — Apple Silicon container, AWS Graviton, ARM cloud VM, Raspberry Pi — got an x86_64 binary that crashed with a loader/rosetta error) and built the download URL straight from the go.mod `go` directive (so a modern `go 1.22`, whose first release is `go1.22.0`, produced a 404; and an ancient `go 1.7` predates modules so `go mod download` couldn't run). It now detects the kernel arch via `uname -m` and installs the latest stable Go — which satisfies the directive's floor (Go is backward-compatible) and matches what the macOS `brew install go` path already did. Also drops `sudo` when already root, so it works in root containers/CI that have no `sudo`. Verified end-to-end on a real arm64 clean-Linux container (READY).
- **A typo'd repo no longer silently sets up your current directory.** A request with no parseable repo now prints an explicit "No repo in your request — setting up the current directory" notice before proceeding, instead of quietly installing runtimes and running the install command wherever you happened to be.
- **No stale file in the published package.** The build now cleans `dist/` first, so the renamed `setup` module no longer ships alongside a leftover compiled `offline.js`.
- **Failure panel no longer suggests the wrong runtime.** A failed `cargo build` could print "Install Go manually" because the hint matched step names by substring ("car**go**"). Hints now use word boundaries, and a dependency-install failure points you at re-running the install command instead of reinstalling a runtime.
- **Rust setup works when rustup has no default toolchain.** With rustup already installed but no default configured (e.g. `--default-toolchain none`), devhelp installed the toolchain but never selected it, so `cargo build` failed with "rustup could not choose a version of cargo." It now pins the toolchain for the project via `rustup override set` (project-scoped, so it won't change your global default, and any `rust-toolchain.toml` still wins).
- **Elixir/Java/Scala/Haskell/.NET/Julia/OCaml/Zig now resolve their runtime on a clean machine too.** Same class as the Go/Rust/Ruby fix, extended to the remaining ecosystems: a runtime installed in one step lands somewhere the fresh `bash -lc` of the deps/build step doesn't have on `PATH`, so `mix deps.get` (asdf shims), `./gradlew build` / `mvn` (SDKMAN JDK), `sbt`/`mill` (Coursier), `stack`/`cabal` (`~/.ghcup/env`), `dotnet restore` (`~/.dotnet`), `julia` (`~/.juliaup/bin`), `dune`/`opam` (active opam switch), and `zig build` (`~/.local/share/zig-*`) failed with "command not found." The runtime wrapper now adds the right PATH/init for each — every preamble is self-guarded, so it's a no-op when the tool is already on PATH (brew/macOS) or its manager isn't installed. `installJulia` and `installScala` also gained the matching preamble where they re-invoke a just-installed manager (`juliaup add`, `cs install`) in a new shell. Validated as syntactically-valid bash, as a clean no-op on a bare `$HOME`, and as correctly resolving the tool when its install dir is present.
